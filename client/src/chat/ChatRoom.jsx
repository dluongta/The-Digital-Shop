...
export default function ChatRoom({
  ...
  // ✅ Giả sử bạn đã thêm hàm revokeMessageApi vào ChatService
  const { getMessagesOfChatRoom, sendMessage, leaveGroupChat, revokeMessageApi } = useApi();

  // ... (Giữ nguyên các useEffect fetch message, auto scroll, getMemberInfo, headerContent, join/leave socket) ...
  ...
  const handleFormSubmit = async (message) => {
    ...
    socket.emit("sendMessageInRoom", {
      _id: res._id, 
      chatRoomId: currentChat._id, senderId: currentUser._id, message,
    });
  ...
  const handleRevokeMessage = async (messageId) => {
    try {
      ...
      // 3. Emit qua Socket cho người khác
